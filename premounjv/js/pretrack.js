var campaignID = "692a67f0bc9773c782ccb20e";
...
var initialSrc = "https://www.vitalitylifestylenews.lat/"+campaignID+"?format=json";
...
setTimeout(function(){
	if (!urlParams.get('rtkcid')) {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				document.querySelectorAll('a').forEach(function(el) {
					if (el.href.indexOf("www.vitalitylifestylenews.lat/preclick")>-1) {
				...
				xhrr.open("GET", "https://www.vitalitylifestylenews.lat/preview?clickid="+rawData.clickid)
				xhrr.send();
			}
		}
		...
		xhrTrack.open("GET", "https://www.vitalitylifestylenews.lat/preview?clickid="+rtkClickID )
		xhrTrack.send();
		document.querySelectorAll('a').forEach(function(el) {
			if (el.href.indexOf("www.vitalitylifestylenews.lat/preclick")>-1) {
				if (el.href.indexOf('?')>-1) {
